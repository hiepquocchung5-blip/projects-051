...
// V5 Premium Auth Screen - Login, Register, Recovery & JWT Integration
...
?>
<style>
    /* Premium Floating Label & Circuit CSS */
...
<div class="flex items-center justify-center min-h-[85vh] w-full px-4 relative z-10 pb-20 mt-10">
    <div class="glass-panel p-8 sm:p-10 rounded-3xl w-full max-w-md relative overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.8)] border border-gray-700/50 backdrop-blur-2xl">
        ...
        <div id="auth-error" class="hidden bg-red-900/30 border border-red-500/50 text-red-500 p-4 rounded-xl text-[10px] mb-6 text-center shadow-inner font-mono break-words font-bold tracking-widest uppercase relative z-10"></div>
        ...
        <div id="captcha-container" class="mb-6 relative z-10 active-state">
            ...
            <div class="flex gap-3">
                <img id="captcha-img" src="" alt="Captcha" class="h-[55px] rounded-xl border border-gray-700 captcha-box bg-[#050507]">
                <input type="text" id="captcha-input" placeholder="8 CHARS" maxlength="8" required class="w-full bg-black/60 border border-gray-700 rounded-xl px-4 text-white outline-none focus:border-premium-gold transition-all shadow-inner font-mono font-bold tracking-widest text-center uppercase text-sm">
            </div>
        </div>
        ...
        <form id="login-form" class="space-y-6 form-section active-state z-10">
            ...
            <div class="relative">
                ...
                <button type="button" onclick="togglePassword('login-pass', 'login-eye')" class="absolute right-4 top-1/2 transform -translate-y-1/2 text-gray-500 hover:text-premium-gold transition-colors">
                    <i data-lucide="eye" id="login-eye" size="18"></i>
                </button>
            </div>
            
            <div class="flex justify-end">
                <button type="button" onclick="toggleAuth('recovery')" class="text-[9px] text-gray-500 hover:text-premium-gold transition-colors font-bold uppercase tracking-widest border-b border-transparent hover:border-premium-gold pb-0.5">Key Recovery Protocol</button>
            </div>
            ...
            <p class="text-center text-[10px] text-gray-500 mt-6 font-bold uppercase tracking-widest">
                New Operative? <span class="text-white cursor-pointer hover:text-premium-gold ml-2 border-b border-transparent hover:border-premium-gold pb-0.5" onclick="toggleAuth('register')">Initialize Profile</span>
            </p>
        </form>
        ...
        <form id="register-form" class="space-y-6 form-section hidden-state z-10">
            <div class="relative">
                <input type="text" id="reg-user" placeholder=" " required class="float-input w-full bg-black/60 border border-gray-700 rounded-2xl px-5 pt-7 pb-3 text-white outline-none focus:border-premium-silver transition-all peer font-bold">
            ...
            <div class="relative">
                <input type="password" id="reg-pass" placeholder=" " required class="float-input w-full bg-black/60 border border-gray-700 rounded-2xl px-5 pt-7 pb-3 pr-12 text-white outline-none focus:border-premium-silver transition-all peer tracking-[0.5em] font-bold">
                ...
                <button type="button" onclick="togglePassword('reg-pass', 'reg-eye')" class="absolute right-4 top-1/2 transform -translate-y-1/2 text-gray-500 hover:text-premium-silver transition-colors">
                    <i data-lucide="eye" id="reg-eye" size="18"></i>
                </button>
            </div>
            <div class="relative">
            ...
            <p class="text-center text-[10px] text-gray-500 mt-6 font-bold uppercase tracking-widest">
                Already registered? <span class="text-white cursor-pointer hover:text-premium-silver ml-2 border-b border-transparent hover:border-premium-silver pb-0.5" onclick="toggleAuth('login')">Return to Login</span>
            </p>
        </form>

        <!-- 3. RECOVERY FORM -->
        <form id="recovery-form" class="space-y-6 form-section hidden-state z-10">
            <p class="text-xs text-gray-400 font-sans leading-relaxed text-center mb-6">Enter your registered email address. If found in the matrix, an extraction link will be dispatched.</p>
            <div class="relative">
                <input type="email" id="recovery-email" placeholder=" " required class="float-input w-full bg-black/60 border border-gray-700 rounded-2xl px-5 pt-7 pb-3 text-white outline-none focus:border-blue-500 transition-all peer font-bold shadow-inner">
            ...
            <button type="submit" id="recovery-btn" class="w-full bg-blue-600 text-white font-black py-4 rounded-xl mt-4 hover:bg-blue-500 transition-all text-xs flex justify-center items-center gap-2 active:scale-95 uppercase tracking-widest shadow-[0_0_20px_rgba(59,130,246,0.3)] border border-blue-500/50">
                Transmit Recovery Link
            </button>
            <p class="text-center text-[10px] text-gray-500 mt-6 font-bold uppercase tracking-widest">
                Protocol aborted? <span class="text-white cursor-pointer hover:text-blue-400 ml-2 border-b border-transparent hover:border-blue-400 pb-0.5" onclick="toggleAuth('login')">Return to Login</span>
            </p>
        </form>
    </div>
</div>
...
<script>
    ...
    async function loadCaptcha() {
        ...
        img.style.opacity = '0.5'; input.value = '';
        try {
            const res = await UrbanixAPI.request('captcha', 'GET');
            img.src = res.data.image; activeCaptchaHash = res.data.hash; activeCaptchaExp = res.data.exp;
            img.style.opacity = '1';
        } catch (err) { console.error("Captcha load failed."); }
    }
    ...
    function toggleAuth(mode) {
        ...
        // Hide all
        [loginForm, regForm, recForm].forEach(el => { el.classList.remove('active-state'); el.classList.add('hidden-state'); });
        ...
        if (mode === 'register') {
            regForm.classList.remove('hidden-state'); regForm.classList.add('active-state');
            captchaBox.classList.remove('hidden-state'); captchaBox.classList.add('active-state');
            ...
            recForm.classList.remove('hidden-state'); recForm.classList.add('active-state');
            captchaBox.classList.remove('active-state'); captchaBox.classList.add('hidden-state'); // Hide captcha for recovery UI
            ...
            loginForm.classList.remove('hidden-state'); loginForm.classList.add('active-state');
            captchaBox.classList.remove('hidden-state'); captchaBox.classList.add('active-state');
    ...
    async function handleNativeAuth(e, action) {
        ...
        payload.captcha = document.getElementById('captcha-input').value;
        ...
        try {
            ...
            lucide.createIcons();
            loadCaptcha(); // Force new captcha on failure
        }
    }
